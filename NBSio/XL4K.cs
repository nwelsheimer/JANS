...
namespace NBSio
{
  public class xl4k
  {
    ...
    public static bool basicExcelExport(DataSet exportTables, string fileName)
    {
      ...
      if (fileName != "")
      {
        try
        {
          ...
          {
            //itterate through all the tables sent
            foreach (DataTable exportSource in exportTables.Tables)
            {
              // get handle to the existing worksheet
              ExcelWorksheet worksheet = xlPackage.Workbook.Worksheets.Add(exportSource.TableName.Length>0 ? exportSource.TableName : "Export");
              int row = startRow;

              //Some global stuff
              int totalColumns = exportSource.Columns.Count;
              Char lastColumn = global5k.LetterToNum(totalColumns);

              //output column headings
              for (int i = 1; i <= totalColumns; i++)
              {
                worksheet.Cells[global5k.LetterToNum(i) + row.ToString()].Value = exportSource.Columns[i - 1].ColumnName;
              }
              row++;
    ...
    public static string GenerateExport(int sessionId, string fileName = "", string fullPath = "")
    {
      ...
      if (fullPath != "")
      {
        ...
        export = global5k.GetData("usp_PB_ExcelExportSummarySKU @sessionId=" + sessionId).Tables[0];
        ...
        string lastColumn = totalColumns + 1 <= 26 ? global5k.LetterToNum(totalColumns + 1).ToString() :
            "A" + global5k.LetterToNum(totalColumns + 1 - 26).ToString();
        ...
        using (ExcelPackage xlPackage = new ExcelPackage(newFile))
        {
          ...
          if (worksheet != null)
          {
            ...
            {
              cell = i <= 26 ? global5k.LetterToNum(i).ToString() : "A" + global5k.LetterToNum(i - 26).ToString();
              worksheet.Cells[cell + "4"].Value = export.Columns[i - 1].ColumnName;
            }
            cell = totalColumns + 1 <= 26 ? global5k.LetterToNum(totalColumns + 1).ToString() :
                    "A" + global5k.LetterToNum(totalColumns + 1 - 26).ToString();
          ...
          if (storeDetail != null)
          {
            ...
            export = global5k.GetData(query).Tables[0];

            //reset globals
            totalColumns = export.Columns.Count;
            lastColumn = totalColumns + 1 <= 26 ? global5k.LetterToNum(totalColumns + 1).ToString() :
                "A" + global5k.LetterToNum(totalColumns + 1 - 26).ToString();
            int row = startRow;
            ...
            for (int i = 1; i <= totalColumns; i++)
            {
              storeDetail.Cells[global5k.LetterToNum(i) + "4"].Value = export.Columns[i - 1].ColumnName;
            }
